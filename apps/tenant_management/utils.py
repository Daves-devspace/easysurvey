...
def filter_meter_readings_for_property(property_obj, month_str: Optional[str] = None) -> List[Dict]:
    ...
    latest_reading_pk_sq = MeterReading.objects.filter(unit=OuterRef("pk"), reading_date__range=(billing_start, billing_end)).order_by("-reading_date").values("pk")[:1]
    prev_current_sq = MeterReading.objects.filter(unit=OuterRef("pk"), reading_date__lt=billing_start).order_by("-reading_date").values("current_reading")[:1]

    units_qs = Unit.objects.filter(property=property_obj).annotate(
        latest_reading_pk=Subquery(latest_reading_pk_sq),
        calculated_previous=Coalesce(Subquery(prev_current_sq, output_field=DecimalField(max_digits=14, decimal_places=2)), Value(Decimal("0.00")), output_field=DecimalField(max_digits=14, decimal_places=2))
    ).prefetch_related(Prefetch("leases", queryset=Lease.objects.filter(is_active=True).select_related("tenant"), to_attr="active_leases_prefetched")).order_by("unit_number")
    ...
    for unit in units_qs:
        ...
        if reading and not is_baseline:
            ...
            status = "pending"
            previous_current = reading.current_reading if is_baseline else unit.calculated_previous
...
def get_tenant_leases_data(tenant):
    ...
    # --- SAFETY FIX: EXCLUDE MIXED PAYMENTS ---
    ...
    for lease in leases_qs:
        ...
        leases_data.append({
            "lease": lease, "lease_obj": lease, "unit": lease.unit, "property": lease.unit.property,
            "rent_amount": lease.unit.rent_amount, "deposit": deposit_required, "deposit_held": deposit_held,
            "status": "Active" if lease.is_active else "Expired",
            "total_invoiced": total_invoiced, "total_paid": total_paid,
            "balance": q(total_invoiced - total_paid), "balance_abs": abs(total_invoiced - total_paid),
            "total_water_usage": total_water_usage, "total_water_amount": total_water_amount,
            "current_meter": current_meter, "previous_meter": previous_meter, "water_lines": water_lines,
        })

    tenant_credit = LedgerEntry.objects.filter(tenant=tenant, invoice__isnull=True, deposit__isnull=True).aggregate(t=Coalesce(Sum("credit"), Value(Decimal("0.00")), output_field=DecimalField()))["t"]
...
def get_property_leases_data(property_obj):
    ...
    if unit_ids:
        ...
        for r in readings:
            if r['unit_id'] not in latest_readings_map: latest_readings_map[r['unit_id']] = r['current_reading']

    invoice_lines = list(InvoiceLine.objects.filter(lease__in=lease_ids).select_related("meter_reading"))
    
    # --- SAFETY FIX: EXCLUDE MIXED PAYMENTS HERE TOO ---
    ...
    for lease in leases_qs:
        ...
        leases_data.append({
            "lease_obj": lease, "tenant": lease.tenant, "unit": lease.unit, "rent_amount": lease.unit.rent_amount,
            "deposit": total_deposit, "status": "Active" if lease.is_active else "Inactive",
            "total_invoiced": total_invoiced, "total_paid": total_paid, "balance": q(total_invoiced - total_paid),
            "total_water_usage": total_water_usage, "total_water_amount": total_water_amount,
