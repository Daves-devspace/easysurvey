...
def filter_meter_readings_for_property(property_obj, month_str: Optional[str] = None) -> List[Dict]:
    ...
    Handles Baseline logic: If usage is 0, it shows as Pending (Previous=Baseline, Current=Empty).
    """
...
def get_tenant_leases_data(tenant):
    ...
    payments = list(Payment.objects.filter(invoice_id__in=invoice_ids).distinct())
    ...
    for lease in leases_qs:
        ...
        # --- DEPOSIT LOGIC UPDATE ---
        # deposit_required: The amount set in the lease contract
        deposit_required = sum(q(d.amount) for d in deposits_by_lease[lid])
        # deposit_held: The actual cash received and held
        ...
        leases_data.append({
            ...
            "deposit": deposit_required,      # Total Required
            "deposit_held": deposit_held,     # Total Paid/Held
