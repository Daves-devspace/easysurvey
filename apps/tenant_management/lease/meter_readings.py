...
class MeterReadingCreateView(CreateView):
    ...
    def get_previous_reading_obj(self):
        ...
        if target_start.month == 12: next_month_start = target_start.replace(year=target_start.year + 1, month=1)
        else: next_month_start = target_start.replace(month=target_start.month + 1)
        return unit.meter_readings.filter(reading_date__lt=next_month_start).exclude(current_reading__isnull=True).order_by("-reading_date", "-id").first()
    ...
    def get_initial(self):
        ...
        prev = self.get_previous_reading_obj()
        if prev: initial["previous_reading"] = prev.current_reading
        return initial
    ...
    def form_valid(self, form):
        ...
        if period_start.month == 12: next_month_start = period_start.replace(year=period_start.year + 1, month=1)
        else: next_month_start = period_start.replace(month=period_start.month + 1)

        prev_obj = unit.meter_readings.filter(reading_date__lt=next_month_start).exclude(current_reading__isnull=True).order_by("-reading_date", "-id").first()
        ...
        if prev_obj:
            ...
            if not form.instance.current_reading: form.instance.current_reading = form.instance.previous_reading
        ...
        if self.object.usage is not None and self.object.usage > 0 and active_lease:
             # MARK AS PROCESSED to prevent duplicate line generation by signal
        ...
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            ...
            item = {
                "unit": unit, "tenant": active_lease.tenant if active_lease else None,
                "reading": self.object, "previous_current": self.object.previous_reading,
                "usage": self.object.usage, "rate": rate_val, "amount": self.object.amount,
            }
    ...
    def form_valid(self, form):
        ...
        if self.request.headers.get("x-requested-with") == "XMLHttpRequest":
            ...
            item = {
                "unit": unit, "tenant": active_lease.tenant if active_lease else None,
                "reading": self.object, "previous_current": self.object.previous_reading,
                "usage": self.object.usage, "amount": self.object.amount, "rate": rate_val,
            }
            row_html = render_to_string("meter_readings/partials/reading_row.html", {"item": item}, request=self.request)
            return JsonResponse({"success": True, "row_html": row_html, "unit_id": unit.id})
        return redirect("property_detail", pk=unit.property_id)
