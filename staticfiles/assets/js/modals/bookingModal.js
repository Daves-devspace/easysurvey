...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function resolveName(primary, serviceId, type) {
    ...
    if (serviceId) {
      const opener = document.querySelector(
        `.open-booking-modal[data-service-id="${serviceId}"]`,
      );
      if (opener?.dataset) {
        if (type === "service" && opener.dataset.serviceName)
          return opener.dataset.serviceName.trim();
        if (type === "client" && opener.dataset.clientName)
          return opener.dataset.clientName.trim();
      }
    }
    return "";
  }
  ...
  function updateBookingRow(bookingId, scheduled_date, dispatch_message) {
    ...
    if (previewSpan) {
      ...
      const short =
        decodedMessage.length > 60
          ? decodedMessage.slice(0, 60) + "…"
          : decodedMessage;
  ...
  function setupResetTemplateButton() {
    document.querySelectorAll(".reset-template-btn").forEach((btn) => {
      btn.addEventListener("click", function () {
        ...
        const realSvcName = resolveName(
          form.dataset.serviceName,
          form.dataset.serviceId,
          "service",
        );
        const realClientName = resolveName(
          form.dataset.clientName,
          form.dataset.serviceId,
          "client",
        );
        ...
        if (dateInput.value) {
          const { full } = buildDispatchMessage(
            realClientName,
            realSvcName,
            dateInput.value,
            isGround,
  ...
  function initBookingForm(form) {
    ...
    const dateInput = form.querySelector(".scheduled-date-input");
    const msgInput = form.querySelector(".message-input");
    const previewSpan = bookingId
      ? document.getElementById(`msg-preview-${bookingId}`)
      : null;

    if (DEBUG)
      console.log("initBookingForm resolved", {
        serviceId,
        bookingId,
        isGround,
      });
    ...
    if (dateInput) {
      dateInput.addEventListener("input", () => {
        ...
        const isGround = form.dataset.isGround === "true";
        const { full, short } = buildDispatchMessage(
          realClientName,
          realSvcName,
          dateInput.value,
          isGround,
        ...
        if (DEBUG)
          console.log("prefill ->", { realClientName, realSvcName, full });
      });
    }
          ...
          if (data.success) {
            ...
            if (isEdit) {
              ...
              const table = document.getElementById(
                `bookings-table-${serviceId}`,
              );
              if (table)
                insertBookingRow(table, {
                  id: data.id,
  ...
  (function setupModalOpener() {
    ...
    document.querySelectorAll(".open-booking-modal").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        modalForm.dataset.clientName = resolveName(
          clientName,
          serviceId,
          "client",
        );
        modalForm.dataset.serviceName = resolveName(
          serviceName,
          serviceId,
          "service",
        );
        ...
        if (action === "edit" && bookingId) {
          ...
          const { full: expectedTemplate } = buildDispatchMessage(
            realClientName,
            realSvcName,
            scheduled,
            isGround === "true",
        ...
        if (clientLabel)
          clientLabel.textContent = modalForm.dataset.clientName
            ? `Client: ${modalForm.dataset.clientName}`
            : "";
        if (serviceLabel)
          serviceLabel.textContent = modalForm.dataset.serviceName
  ...
  document
    .querySelectorAll(".booking-form:not(#bookingModal form.booking-form)")
    .forEach((f) => initBookingForm(f));
  document
    .querySelectorAll(".preview-with-tooltip")
